...
* **strace**'ing shows that is trying to acquire a lock (i.e. **futex(...)**)
...
* allows taking a **core dump** (a state of process memory at a specific moment of time) to analyze it later (e.g. after crash)
...
* the very same mechanism is [used](https://github.com/python/cpython/blob/master/Tools/gdb/libpython.py) for debugging of CPython, that adds some confusion when searching for "gdb python" on Google 
